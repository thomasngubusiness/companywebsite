/* Worldwide country list with international dialing codes.
   Used by the enquiry form: select a country -> phone prefills its (+code). */
window.COUNTRIES = [
...
["Russia","+7"],["Rwanda","+250"],["Saint Kitts and Nevis","+1"],["Saint Lucia","+1"],["Samoa","+685"],
["San Marino","+378"],["Saudi Arabia","+966"],["Senegal","+221"],["Serbia","+381"],["Seychelles","+248"],
...
["Vanuatu","+678"],["Vatican City","+39"],["Venezuela","+58"],["Vietnam","+84"],["Yemen","+967"],
["Zambia","+260"],["Zimbabwe","+263"]
];

/* Auto-init: populate any <select data-country> (Malaysia first, then A–Z) and
   prefill the form's phone field with the chosen country's dial code. */
(function () {
  function dialFor(name) {
    for (var i = 0; i < window.COUNTRIES.length; i++) if (window.COUNTRIES[i][0] === name) return window.COUNTRIES[i][1];
    return '';
  }
  function build(sel) {
  ...
  function init() {
    var sel = document.querySelector('select[data-country]');
    if (!sel) return;
    build(sel);
    var form = sel.closest('form');
    var phone = form && form.querySelector('input[name="phone"]');
    sel.addEventListener('change', function () {
      var dial = dialFor(sel.value);
      if (!phone || !dial) return;
      // Replace any existing leading dial code, keep whatever the user typed after it.
      var rest = phone.value.replace(/^\s*\+\d{1,4}\s*/, '').trim();
      phone.value = dial + (rest ? ' ' + rest : ' ');
      phone.focus();
      var len = phone.value.length; try { phone.setSelectionRange(len, len); } catch (e) {}
    });
  }
